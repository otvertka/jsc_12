...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    // Timer
    if (timer) clearInterval(timer); // очистить таймер, если был
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    // Reset timer
    clearInterval(timer);
    timer = startLogOutTimer(); // перезаписываем таймер
  }
});
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    setTimeout(function () {
      ...
      // Reset timer
      clearInterval(timer);
      timer = startLogOutTimer(); // Добавляем таймер при Переводе и Кредите
    }, 2500);
  }
